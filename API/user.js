...
router.post('/login', async (req, res) => {
    ...
    const token = jwt.sign({ _id: users._id }, 'thisistoken', { expiresIn: '24h' })
...
router.patch('/updateuser', auth, async (req, res) => {
    ...
    await User.findByIdAndUpdate(ruser._id, duser).then((data) => {
...
router.post('/logout', auth, async (req, res) => {
    ...
    const remainToken = await ruser.tokens.filter(tokenobj => tokenobj.token !== token)
...
router.delete('/userdelete', auth, async (req, res) => {
    const ruser = req.user
    await User.findByIdAndDelete(ruser._id).then(() => {
